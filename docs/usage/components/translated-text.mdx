...
Doing something like above will allow you to simply use `<Text />` as normal within your app without the need to even consider translations (except for certain circumstances where you need to provide `context` or `disableTranslation`).
...
This field allows you to clarify to provide additional context to translations when you feel it's appropriate.
...
This field is used to disable the translation of the string within the `<TranslatedText />`. This can be useful if you've created a custom `<Text />` component as mentioned above and don't want to translate a specific text element.
</ResponseField>
