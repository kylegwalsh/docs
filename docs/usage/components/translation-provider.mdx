...
You should only import this one time at the top of your application in order to enable translations.
...
Most the time you do not need to provide this, but if you are not happy with the translations and want to provide some guidance, this is where you would do it.
...
This setting is enabled by default and basically says: if the app was written in "en-US", don't translate it to "en-GB". So basically, we just look at the "en" portion of your `defaultLanguage` field and determine whether something needs to be translated.

We do this so that you don't waste translation credits translating "English" into another dialect of "English".

If you set this to **false**, we will translate even when the language is the same if the country / dialect is different. This could be useful in some langauges if there are major differences in dialects.
</ResponseField>
...
Whether we should show additional console.logs. This can be useful when debugging why things are translated certain ways.
</ResponseField>