...
The simplest way to translate text is to simply wrap some text with the `<TranslatedText />` component. It will detect the user's locale via their device settings and generate a translation in real-time. Afterwards, it stores the translation in a locale cache to ensure everything stays as fast as possible.
...
See the [TranslatedText documentation](/docs/usage/components/translated-text) for more details.
</Tip>
