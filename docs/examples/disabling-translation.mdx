...
If we were transalting to Spanish, the above would normally render something "Tu cuenta bancaria se llama cuenta de prueba". As you can see, "test account" was completely replaced.
...
In the above example, we would normally attempt to re-translate the count whenever it changes, but by adding `disableTranslation` will not attempt to translate the text.
